<template>
  <main id="Invoice-page">
    <h1>Edit Return Invoice</h1>
    <p>Welcome to Edit Return Invoice page</p>
          ...
          <div class="top-invoice-section">
            <div class="invoice-top-section-details-left">
              <div class="invoice-custom-id">
                <label>Reurn Invoice No </label>
                <label style="color: var(--primary)">{{
                  invoiceInfo.custom_return_invoice_id
                }}</label>
              </div>
            ...
            <div class="invoice-top-section-details-right">
              <div class="form-invoice-date">
                ...
                <input
                  ...
                  :class="[
                    'form-input-holder',
                    errors.return_invoice_date ? 'is-invalid' : '',
                  ]"
                  v-model="invoiceInfo.return_invoice_date"
                />
                <div v-if="errors.return_invoice_date" :class="['errorText']">
                  <div
                    class="errorText-inner"
                    v-for="error in errors.return_invoice_date"
                    v-bind:key="error.id"
                  >
                    <ul>
                      <li>{{ error }}</li>
...
export default {
  ...
  setup() {
    ...
    const getInvoice = () => {
      axios
        .get("return-invoice/" + invoice_id_from_url.value)
        .then((response) => {
          invoiceInfo.custom_return_invoice_id =
            response.data.return_invoice.custom_return_invoice_id;
          invoiceInfo.id = response.data.return_invoice.id;
          invoiceInfo.customer_id = response.data.return_invoice.customer_id;
          invoiceInfo.customer_name =
            response.data.return_invoice.customer_name;
          invoiceInfo.note = response.data.return_invoice.note;
          invoiceInfo.return_invoice_date =
            response.data.return_invoice.return_invoice_date;
          invoiceInfo.due_date = response.data.return_invoice.due_date;
          invoiceInfo.sub_total = response.data.return_invoice.sub_total;
          invoiceInfo.discount = response.data.return_invoice.discount;
          invoiceInfo.tax_amount = response.data.return_invoice.sub_total;
          invoiceInfo.grand_total = response.data.return_invoice.grand_total;
    ...
    const editInvoice = () => {
      const invoice_slip = {
        ...
        return_invoice_date: invoiceInfo.return_invoice_date,
        note: invoiceInfo.note,
        discount: invoiceInfo.discount,
        id: invoiceInfo.id,
      };
      axios
        .post("/return-invoice/edit", invoice_slip)
        .then((response) => {
